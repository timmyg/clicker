	...
	},
	{
		...
		expires: {
			...
			defaultExpires: (x) => {
				...
				return moment(x.end).add(2, 'hours').toDate();
			}
		}
	}
);
...
function rank(program: Program) {
	...
	if (program.subcategories) {
		program.subcategories.includes('Playoffs') || program.subcategories.includes('Playoff')
			? (totalPoints += 5)
			: null;
...
module.exports.syncAirtable = RavenLambdaWrapper.handler(Raven, async (event) => {
	...
	for (const region of allRegions) {
		...
		for (const result of results) {
			const allExistingGames = await base(airtablePrograms).select({ fields: [ 'programmingId' ] }).all();
...
async function getAirtableProgramsInWindow(hasGameAttached, hoursAgo = 4, hoursFromNow = 4) {
	...
	const sixMinutesAgo = moment().subtract(6, 'm').toISOString();
...
module.exports.syncAirtableUpdates = RavenLambdaWrapper.handler(Raven, async (event) => {
	...
	for (const airtableProgram of updatedAirtablePrograms) {
		...
		const programs = await dbProgram.query('programmingId').eq(programmingId).exec();
		...
		terms.map((term) => {
			...
			programs.forEach((p: any) => {
				if (isProperty) {
					...
					const titleHasTerm = p.get('title').toLowerCase().includes(term);
					if (titleHasTerm) {
						if (!p.get('rating')) {
							p.set('rating', rating);
							rated.push(p);

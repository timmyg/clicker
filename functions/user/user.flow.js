...
module.exports.referral = RavenLambdaWrapper.handler(Raven, async (event) => {
	...
	const user = await User.queryOne('id').eq(userId).exec();

	const referrerUser = await User.queryOne('referralCode').eq(code).all().exec();
...
module.exports.wallet = RavenLambdaWrapper.handler(Raven, async (event) => {
	const userId = getUserId(event);
	let user = await User.queryOne('id').eq(userId).exec();
	console.log({ user });
...
module.exports.removeCard = RavenLambdaWrapper.handler(Raven, async (event) => {
	...
	const { stripeCustomer } = await User.queryOne('id').eq(userId).exec();
...
module.exports.alias = RavenLambdaWrapper.handler(Raven, async (event) => {
	...
	// get existing users
	const fromUser = await User.queryOne('id').eq(fromId).exec();
	const toUser = await User.queryOne('id').eq(toId).exec();
...
async function getToken(phone) {
	const user = await User.queryOne('phone').eq(phone).all().exec();
	if (user) {
		const { id } = user;

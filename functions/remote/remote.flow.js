...
class LosantApi {
	...
	async sendCommand(name, losantId, payload) {
		try {
			...
			return await this.client.device.sendCommand(params).then(console.info).catch(console.error);
		} catch (error) {
...
module.exports.command = RavenLambdaWrapper.handler(Raven, async (event) => {
	try {
		...
		if (source === 'app') {
			...
			text = `${text} to ${reservation.program.title} on ${reservation.program
				.channelTitle} (${reservation.minutes} mins, TV: ${reservation.box.label}, user: ${userId.substr(
				userId.length - 5
			)})`;
			...
			userId = 'system';
			const text = `*${eventName}* @ ${reservation.location.name} to ${channel} on *Zone ${reservation.box
				.zone}*`;

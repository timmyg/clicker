...
module.exports.controlCenterDailyInit = RavenLambdaWrapper.handler(
  Raven,
  async event => {
...
module.exports.syncLocationsBoxes = RavenLambdaWrapper.handler(
  Raven,
  async event => {
    // TODO pull actual regions
    const regions = ["cincinnati"];
...
// ccv1
module.exports.updateAllGamesStatus = RavenLambdaWrapper.handler(
  Raven,
  async event => {
    try {
...
module.exports.controlCenter = RavenLambdaWrapper.handler(
  Raven,
  async event => {
    const base = new Airtable({ apiKey: process.env.airtableKey }).base(
      process.env.airtableBase
    ...
    if (games.length) {
      ...
      for (const game of games) {
        ...
        if (waitOn && waitOn.length) {
          ...
          const dependencyGame = await base(airtableControlCenterV1).find(
            dependencyGameId
          );
          ...
          const dependencyChannel: string = dependencyGame.get(
            "Channel Number"
          );
          const dependencyZones: number[] = dependencyGame.get("TV Zones");
          const dependencyGameStatus: string = dependencyGame.get(
            "Game Status"
          );
          ...
          if (lockedUntil === "Blowout" && !blowout && !gameOver) {
            console.log(
              `waiting on blowout:`,
              dependencyGame.get("Title (Calculated)")
            );
            waitingCount++;
        ...
        for (const location of locations) {
          ...
          if (
            gamePackage &&
            (!location.packages || !location.packages.includes(gamePackage))
          ) {
          ...
          const boxes = location.boxes.filter(
            b =>
              zones.includes(b.zone) &&
              (!b.reserved ||
                (b.reserved && moment(b.end).diff(moment().toDate()) < 0))
          );

...
module.exports.all = RavenLambdaWrapper.handler(Raven, async event => {
  ...
  const milesRadius =
    event.queryStringParameters && event.queryStringParameters.miles
      ? event.queryStringParameters.miles
      : null;
  ...
  allLocations.forEach((l, i, locations) => {
    if (l.boxes) {
      l.openTvs = l.boxes.every(
        b => !b.reserved || moment(b.end).diff(moment().toDate()) < 0
      );
    }
  });
...
module.exports.get = RavenLambdaWrapper.handler(Raven, async event => {
  ...
  if (location.boxes) {
    location.boxes.forEach((o, i, boxes) => {
      ...
      if (
        boxes[i].reserved &&
        moment(boxes[i].end).diff(moment().toDate()) < 0
      ) {
...
module.exports.setBoxes = RavenLambdaWrapper.handler(Raven, async event => {
  ...
  for (const box of boxes) {
    ...
    const existingBox =
      ...
      location.boxes.find(
        locationBox =>
          locationBox.ip === box.ip &&
          locationBox.clientAddress === box.clientAddress
      );
    if (!existingBox) {
      box.id = uuid();
      // box.active = true;
...
module.exports.setBoxReserved = RavenLambdaWrapper.handler(
  Raven,
  async event => {
...
// called from antenna
module.exports.saveBoxesInfo = RavenLambdaWrapper.handler(
  Raven,
  async event => {
      ...
      if (originalChannel !== major) {
        ...
        if (!!location.boxes[i].zone || location.boxes[i].appActive) {
          const text = `Manual Zap @ ${location.name} (${
            location.neighborhood
          }) from *${originalChannel}* to *${major}* (Zone ${
            location.boxes[i].zone
          })`;
...
module.exports.identifyBoxes = RavenLambdaWrapper.handler(
  Raven,
  async event => {
    const { id } = getPathParameters(event);
...
module.exports.connected = RavenLambdaWrapper.handler(Raven, async event => {
  ...
  if (!!location) {
    ...
    const text = `Antenna Connected @ ${location.name} (${
      location.neighborhood
    })`;
    await new Invoke()
      .service("notification")
...
module.exports.disconnected = RavenLambdaWrapper.handler(Raven, async event => {
  ...
  if (!!location) {
    ...
    const text = `Antenna Disconnected @ ${location.name} (${
      location.neighborhood
    })`;
    await new Invoke()
      .service("notification")
...
module.exports.checkAllBoxesInfo = RavenLambdaWrapper.handler(
  Raven,
  async event => {
    // const { id } = getPathParameters(event);
    let allLocations: Venue[] = await dbLocation.scan().exec();
...
module.exports.controlCenterLocationsByRegion = RavenLambdaWrapper.handler(
  Raven,
  async event => {
...
module.exports.updateAllBoxesPrograms = RavenLambdaWrapper.handler(
  Raven,
  async event => {
...
module.exports.updateBoxInfo = RavenLambdaWrapper.handler(
  Raven,
  async event => {
    const { id: locationId, boxId } = getPathParameters(event);
...
class ControlCenterProgram {
  ...
  isMinutesFromNow(minutes: number) {
    return (
      moment.duration(moment(this.fields.start).diff(moment(), "minutes")) <
      minutes
    );
  }
}

module.exports.controlCenterV2 = RavenLambdaWrapper.handler(
  Raven,
  async event => {
...
function filterPrograms(
  ...
  console.info(
    `filtered programs after looking at currently showing: ${ccPrograms.length}`
  );

  // remove channels that location doesnt have
  const excludedChannels =
    location.channels &&
    location.channels.exclude &&
    location.channels.exclude.map(channel => parseInt(channel, 10));
  console.info({
  ...
  if (excludedChannels && excludedChannels.length) {
    ccPrograms = ccPrograms.filter(
      ccp => !excludedChannels.includes(ccp.db.channel)
    );
  }
  console.info(
    `filtered programs after looking at excluded: ${ccPrograms.length}`
  );
...
module.exports.controlCenterV2byLocation = RavenLambdaWrapper.handler(
  Raven,
  async event => {
    const { id: locationId } = getPathParameters(event);
    const location: Venue = await dbLocation
      ...
      .exec();
    console.info(
      `Running Control Center for: ${location.name} (${location.neighborhood})`
    );
    ...
    for (const program of ccPrograms) {
      ...
      console.info(
        `trying to turn on: ${program.fields.title} (${program.db.channel}) *${
          program.fields.rating
        }*`
      );
      ...
      if (selectedBox) {
        ...
        availableBoxes = availableBoxes.filter(
          b => b.id !== (selectedBox && selectedBox.id)
        );
...
module.exports.updateAirtableNowShowing = RavenLambdaWrapper.handler(
  Raven,
  async event => {
    let locations: Venue[] = await dbLocation.scan().exec();
    ...
    boxes
      ...
      .filter(
        b =>
          ...
          (b.channelSource === "manual" &&
            moment(b.channelChangeAt).diff(moment(), "minutes") <
              -manualChangeMinutesAgo)
      )
      ...
      .filter(
        b =>
          b.channelSource !== "manual" ||
          (b.channelSource === "manual" &&
            moment(b.channelChangeAt).diff(moment(b.program.start), "minutes") <
              -manualChangeBuffer)
      )
  );
}

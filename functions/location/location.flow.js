	...
	for (const location of locations) {
		for (const box of location.boxes) {
			if (box.channel) {
				...
				await updateLocationBox(location.id, boxIndex, box.channel, 0, null, program);
...
async function tune(location: Venue, box: Box, channel: number) {
	...
	await new Invoke().service('remote').name('command').body({ reservation, command, source }).async().go();
}
	...
	boxes.forEach((b) => {
		boxStatuses.push(
			new BoxStatus({
				...
				ended:
					!!b.program && !!b.program.game && !!b.program.game.liveStatus && b.program.game.liveStatus.ended,
				blowout:
					!!b.program && !!b.program.game && !!b.program.game.liveStatus && b.program.game.liveStatus.blowout,

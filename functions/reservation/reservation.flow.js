...
module.exports.create = RavenLambdaWrapper.handler(Raven, async event => {
  ...
  if (
    !tv ||
    !tv.appActive ||
    (tv.reserved && moment(tv.end).unix() > moment().unix())
  ) {
    console.log(tv);
...
module.exports.update = RavenLambdaWrapper.handler(Raven, async event => {
  ...
  const updatedMinutes =
    originalReservation.minutes + updatedReservation.minutes;
...
function calculateReservationEndTime(reservation) {
  ...
  const initialEndTimeMoment = reservation.end
